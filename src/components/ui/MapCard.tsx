    ...
    <Card onPress={onPress} className="mb-3">
      ...
      <View className="flex-row items-start justify-between">
        <View className="flex-1 pr-3">
          ...
          {distance ? (
            <Text className="text-yellow-600 text-sm mt-2 font-medium">{distance}</Text>
          ) : null}
        </View>
        <View className="bg-yellow-500/20 p-3 rounded-full">
          <Navigation size={20} color={brand.primaryDark} />
        </View>
      </View>
    </Card>
  )
